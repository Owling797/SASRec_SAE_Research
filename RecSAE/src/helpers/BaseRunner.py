...
class BaseRunner(object):
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		model.train()
		loss_lst = list()
		# Use num_workers=0 to avoid serialization issues
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False, prediction_label = 'prediction') -> np.ndarray:
		...
		predictions = list()
		loss_list = []
		# Disable multiprocessing by setting num_workers=0 to avoid serialization issues
		dl = DataLoader(dataset, batch_size=self.eval_batch_size, shuffle=False, num_workers=0,
						collate_fn=dataset.collate_batch, pin_memory=self.pin_memory)
		with torch.no_grad():  # Add this to ensure no gradients are computed
