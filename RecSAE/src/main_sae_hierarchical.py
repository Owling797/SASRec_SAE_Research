...
# Add import for direct access to modules
...
def train_sae(args, model, runner, data_dict):
	...
	# Check existence of directories for model
	...
	if os.path.exists(args.model_path):
		...
		# Search for models in directory with "recsae_Hierarchical"
		hierarchical_pattern = os.path.join(os.path.abspath('../model/*Hierarchical*'), '*.pt')
		hierarchical_models = glob.glob(hierarchical_pattern)
		
		# If nothing found, search in base model directory
		if not hierarchical_models:
			base_pattern = os.path.join(os.path.abspath('../model/*/'), '*.pt')
			hierarchical_models = glob.glob(base_pattern)
			logging.info(f"Searching for models in base directory: {base_pattern}")
		
		# If models found, load the first one
		if hierarchical_models:
			try:
				model_path = hierarchical_models[0]
	...
	# For saving metrics by epoch
	metrics_by_epoch_dev = []
	metrics_by_epoch_test = []
	
	if args.train > 0:
		# Save original train method for later use
		original_train = runner.train
		
		# Redefine train method for metric collection
		...
		# Replace train method with our version for metric collection
		runner.train = train_with_metrics
		
		# Call training with modified train version
		...
		try:
			...
			# Create DataFrame for saving metrics
			metrics_data = {'metric': target_metrics}
			
			# Add columns for each epoch (dev)
			...
			# Add columns for each epoch (test)
			...
			# Create separate file for each metric with columns epoch, dev, test
			for metric in target_metrics:
	...
	# Display final summary table of metrics, if training was performed
	if args.train > 0 and len(metrics_by_epoch_dev) > 0:
		best_epoch_idx = -1  # Use last epoch by default
		
		# Find best epoch by HR@5 metric on validation
		hr5_key = 'hr@5'
		hr5_key_upper = 'HR@5'
		...
		# Display final metrics table for best epoch
...
if __name__ == '__main__':
	...
	init_parser.add_argument('--model_name', type=str, default='SASRec_SAE_Hierarchical', help='Choose a model to run.')
	...
	# Use full model name instead of splitting into base model
	...
	# Use base model SASRec for log file name
	...
	# Check all necessary paths again and print them to log
	...
	def ensure_dir_exists(file_path):
		"""Creates all parent directories for specified file path"""
	...
	# Check if directories exist, and if not, try to create them with absolute paths
	...
	if not os.path.exists(model_dir):
		...
		try:
			...
			# Last case - save in current directory
	...
	try:
		# Add exception handling for main()
		main()
	except Exception as e:
		...
		# Ensure all outputs are written
		for handler in logging.root.handlers:
			handler.flush()
